...
There’s a strong need for better text support on Canvas, and a string desire for a more general solution that incorporates HTML content into a canvas rendering context. Conceptually, this use case has similarity to a mobile WebView within a canvas drawing application.

* **Styled and Laid Out Content in Canvas:** This includes both visual features like multi-line styled text, grid, tables etc. but also the possibility to support the same level of user interaction as the rest of the web (scrolling, interactions, accessibility, indexability, translate, find-in-page, IME input, spellcheck, autofill, etc).
...
* What is the element's intrinsic size for drawImage etc?
* The same code infrastructure for drawElement may make it possible to implement the [```element()```](https://developer.mozilla.org/en-US/docs/Web/CSS/element) image source in html (used in background rendering).
