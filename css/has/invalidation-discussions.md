...
When we have some discussions about a limitation that disallow something, it would be better to minimize the variations to focus the complexity or performance impact of that limitation. Listing exclusive criteria by which `:has()` expressions can be divided into two distinct group (allowing something/disallowing something) will be helpful to focus the limitation itself. We can focus complexity or performance impact when we try to support cases that allowing something, or we can focus complexity or performance impact when we have another approach that support opposite cases(disallowing something). And based on the complexity or performance impact, we can say that, when we have a limitation of disallowing something, we may have this kind of advantages.
